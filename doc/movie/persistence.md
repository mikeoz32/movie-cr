...
class Counter < Movie::EventSourcedBehavior(CounterCommand, Added, CounterState)
  ...
  protected def handle_command(
    ...
    case command
    when Add
      persist(Added.new(command.amount))
    when GetCounter
...
extension.register_entity(Counter) do |id, store|
  Counter.new(id.persistence_id, store)
end
counter = extension.get_entity_ref_as(CounterCommand, Movie::Persistence.id(Counter, "counter-1"))
```

A runnable version is available at [`examples/persistence_example.cr`](../../examples/persistence_example.cr).

`persist(event)` appends one event. `persist_all(events)` appends every event from one command in one transaction. `none` performs no storage write. `stop` requests graceful actor termination. `then_run` callbacks run only after a successful journal write and state application; use them for acknowledgements and other externally visible command side effects.

`apply_event` must be deterministic and return a new logical state. Movie evaluates the candidate state before writing, persists the complete event batch atomically, then publishes the new in-memory state.
...
Recovery and write hooks are available for metrics and logging: `on_recovery_completed`, `on_recovery_failure`, and `on_persist_failure`. A timed-out write is not cancelled at SQLite. The actor restarts and recovers, while optimistic revisions prevent a retry from becoming an unnoticed duplicate write.
...
JSON is written directly into the one `String` required for SQLite text binding; no second intermediate JSON string is built.
