...
PostgreSQL supplies shared durable storage and node-to-node recovery, but not actor ownership. Movie cluster membership supplies discovery, reachability, and explicit member lifecycle; it still does not provide sharding/placement, persistence-id ownership, relocation, or split-brain write fencing. PostgreSQL replication and failover are deployment responsibilities; Movie reconnects after a lost established connection but does not provision or promote database replicas.
